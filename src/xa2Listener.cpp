...
{
	m_listener.pCone->InnerAngle = RETURN_MIN(innerAngle, X3DAUDIO_2PI);			// コーンの角度 (ラジアン単位)
	m_listener.pCone->OuterAngle = X3DAUDIO_2PI - m_listener.pCone->InnerAngle;		// 
	m_listener.pCone->InnerVolume = RETURN_MIN(innerVolume, 2.0f);					// ボリューム スケーラ
	m_listener.pCone->OuterVolume = RETURN_MIN(outerVolume, 2.0f);					// 
...
{
	m_listener.pCone->InnerAngle = RETURN_MIN(innerAngle, X3DAUDIO_2PI);					
	m_listener.pCone->OuterAngle = X3DAUDIO_2PI - m_listener.pCone->InnerAngle;		
}
...
{
	m_listener.pCone->InnerVolume = RETURN_MIN(innerVolume, 2.0f);							
	m_listener.pCone->OuterVolume = RETURN_MIN(outerVolume, 2.0f);							
}
...
{
	m_listener.pCone->InnerLPF = RETURN_MIN(innerLPF, 1.0f);								
	m_listener.pCone->OuterLPF = RETURN_MIN(outerLPF, 1.0f);								
}
...
{
	m_listener.pCone->InnerReverb = RETURN_MIN(innerReverb, 2.0f);							
	m_listener.pCone->OuterReverb = RETURN_MIN(outerReverb, 2.0f);							
}
