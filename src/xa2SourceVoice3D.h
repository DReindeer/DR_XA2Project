...
public:
	...
	// 速度
	void SetVelocity(const D3DXVECTOR3 &move) { m_emitter.Velocity = move; }

	// 上方向
	void SetTop(D3DXVECTOR3 top) { D3DXVec3Normalize(&top, &top); m_emitter.OrientTop = top; }

	// 前方向
	void SetFront(D3DXVECTOR3 front) { D3DXVec3Normalize(&front, &front); m_emitter.OrientFront = front; }
