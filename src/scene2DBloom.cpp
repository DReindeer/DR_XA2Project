...
void CScene2DBloom::Draw()
{
	...
	// Gバッファ書き込み終了
	pDefferd->DrawCombineBloomEnd();
}

//--------------------------------------------------------------------------------
// 作成処理
//--------------------------------------------------------------------------------
CScene2DBloom *CScene2DBloom::Create(const D3DXVECTOR3 &pos, const D3DXVECTOR3 &rot, const D3DXVECTOR3 &size, const std::string &strFilePass, int priority, CTexManager *pUseTexManager)
{
	// スレッドセーフ(ロック)
#ifdef THREAD_ON
	std::unique_lock<std::recursive_mutex> locker = CManager::Locker();
	...
	// CScene2DBloomの生成
	CScene2DBloom *pScene2D;
	pScene2D = new CScene2DBloom(priority);
